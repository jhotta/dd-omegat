--- title: Outlier Detection kind: guide listorder: 16 sidebar:  nav:  - header: Outlier Detection  - text: Show Outliers in Dashboards  href: "#dashboards"  - text: Alert on Outliers  href: "#alerts"  - text: Algorithms & Parameters
  href: "#algorithms"
---
...
`outliers`The  query function, when applied to your query, will return the usual results but with outlier series marked.
...
`outliers`To set up an outlier detection graph for your data add a metric to the graph showing all series in the groups. Then apply the outlier detection algorithm by adding the  function on your data. After applying the function, any outlier series will be colored with a bold, warm palette, while all other series will be colored with a lightweight, greyscale color palette.
...
`tolerance`The only parameter we take is , the constant by which the initial threshold is multiplied to yield DBSCAN’s distance parameter 𝜀. Here is DBSCAN with a tolerance of 3.0 in action on a pool of Cassandra workers:
