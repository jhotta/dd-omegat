...
[other-3[][billing-3]***You can read more about billing here.***
...
Your query runs a full text search of events and their comments. You can also target certain event properties, such as the event source or status, by using the following search prefixes:****`pup@datadoghq.com` Find all events with comments by pup@datadoghq.com.****`github,chef` Show events from Github and Chef.****`env-prod,db` Show events tagged with #env-prod AND #db.****`db1.myapp.com,db2.myapp.com` Show events from db1.myapp.com OR db2.myapp.com.****`error` Show only error status events. (**supports:** 'error', 'warning', 'success')****`low` Show only low-priority events. (**supports:** 'low' or 'normal'. defaults to 'all')****`claimed` Show only claimed incidents. (**supports:** 'open', 'claimed', 'resolved', or 'all')
...
[][events-2]Yes! To get started you need to go the API tab in the settings page, create one or more API emails.

For a monitoring tool that does not allow you to edit the body of the email you need to create a plain-text email, and have that system or tool send alarms or notifications to this email.

[][events-3]For systems that allow you to edit the content of the notification emails, you may use a JSON email. In the body of an email sent to a JSON API email you need to include a well formatted JSON event request as per our existing events API. For more details about our events API, visit our API documentation.

Here is an example:

[]~~~ {  "title": “Host CPU above 75% for 5 minutes",  "text": "Host CPU has been above 75% for the last 5 minutes …etc",  "priority": "normal",  "tags": "vsphere", "env:prod", "host:i-a4f761f0", "role:admin",  "alert_type": "error" }
~~~

#### What formatting is available for event text?
{: #eventformat}

[][events-4]We have adopted Daring Fireball's Markdown throughout the site. To find out more about Markdown, visit the Markdown docs.
...
`app.foo.bar{env:staging}``app.foo.baz{env:staging}``by {host}`To graph the sum of  and  grouped , write a graph query that looks like:
...
~~~ "q": "sum:system.io.rkb_s{device:sda}*1024, sum:system.io.rkb_s{device:sdb} *1024, sum:system.io.rkb_s{device: sdc}*1024"*
~~~
...
`ewma_20(exception.invalid{*})`[][graphing-1]You can apply smoothing averages to your series by droping to the JSON editor and adding ‘ewma’, for example: add any of ewma_x(…) where x can be 5, 10, 20 around your series, e.g. . ewma stands for exponentially-moving average and the full list of functions you can apply is here.


#### Can I stack CPU metrics on the same graph?
{: #stack-cpu-metrics}

[][graphing-2][][graphing-3]Check out our documentation on stacked series. The metric explorer just does one metric per graph, but you can see a stacked CPU graph on the overview page by clicking any host here.
...
[][graphing-4]Vacuum is particularly resource-intensive though, so you might want Datadog events for each time they run so you can correlate metrics and other events with vacuums. You can do this with the dogwrap command line tool provided by the datadog python client library:

0 0 * * * /path/to/dogwrap -n "Vacuuming mytable" -k $API_KEY --submit_mode errors "psql -c 'vacuum verbose my_table' 2>&1 /var/log/postgres_vacuums.log"


`--submit_mode all`This will call the command at the end of the script and send Datadog events if it exits with a non-zero exit code (i.e. an error).  will send events on every run.

`pip install datadog`(To get the python client lib you can install it with ).
...
[][integrations-3]Yes! The agent is entirely open source and can be found on our Github project page.
...
[][metrics-1]You can submit your custom metrics with the DogStatsD client. You can read more about this here.


#### Why is my counter metric showing decimal values?

[][metrics-2]StatsD counters are normalized over the flush interval to report per-second units. You can read more about this here.


#### Is there a way to submit metrics from my log data?

[][metrics-3]Yes there is! We detail log parsing here.
...
`system.disk.in_use`The Datadog Agent emits a metric named  which will give you disk usage as a percentage.
...
[][other-1]The admin of the account should enter the email addresses of team members here. Some team best practices are as follows:
